...
class AbstractImage(object):
    ...
    def getPixel1D(self, i):
        x = i%self.width
        y = i//self.width
        return self.getPixel(x,y)

    def setPixel1D(self, i, pixel):
        x = i%self.width
        y = i//self.width
...
if __name__ == '__main__':
    ...
    imageFileName = input("enter the image file to open: ")
    oImage = FileImage(imageFileName)

    win = ImageWin("My Window", oImage.getWidth()*2+1, oImage.getHeight())
    oImage.draw(win)

    myImage = EmptyImage(oImage.getWidth()+10, oImage.getHeight())
    
    for x in range(0, oImage.getWidth()//2):
        for y in range(oImage.getHeight()):
            myImage.setPixel(x, y, oImage.getPixel(x,y))
    for x in range(oImage.getWidth()//2, oImage.getWidth()//2+10):
         for y in range(oImage.getHeight()):
            myImage.setPixel(x, y, Pixel(0,0,0))
    for x in range(oImage.getWidth()//2+10, oImage.getWidth()):
         for y in range(oImage.getHeight()):
            myImage.setPixel(x, y, oImage.getPixel(x-10,y) )
        
    myImage.setPosition(oImage.getWidth()+1,0)
    myImage.draw(win)


     ## blankImage = EmptyImage(100,100)
    ## for i in range(blankImage.getNumPixels()):
    ##     blankImage.setPixel1D(i, Pixel(0, 0, 255))
    ## blankImage.draw(win)
        
    ## ## for row in range(myImage.getHeight()):
    ## ##     for col in range(myImage.getWidth()):
    ## ##          v = myImage.getPixel(col,row)
    ## for i in range(myImage.getNumPixels()):
    ##     v = myImage.getPixel1D(i)
    ##     v.red = 255 - v.red
    ##     v.green = 255 - v.green
    ##     v.blue = 255 - v.blue
    ##     myImage.setPixel1D(i, v)
    ##     ## myImage.setPixel(col,row,v)
    ## myImage.setPosition(myImage.getWidth()+1,0)
    ## myImage.draw(win)
    ## print(win.getMouse())
    ## myImage.save('testIm.gif')
